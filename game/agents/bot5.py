...
'''
Bot 5 is Bot 2, except that the probabilities of where the crew members are account for the fact that there are two of them.
We take the probability map that Bot 4 uses, however we collapse it into a 2D map with each cell i holding the probability:
    P(C1 = i or C2 = j | data)
'''
class Bot5(Bot):
    alienPbbMap = None
    ...
    def _utility(self, pos):
        ...
        return self.crw * self.crewmateUtility[x][y] + self.aln * self.ape.alienPbbMap[x][y] + expWeight
